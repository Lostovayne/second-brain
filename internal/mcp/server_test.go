...
func TestMCPServer_HandshakeAndTools(t *testing.T) {
	...
	if len(listResult.Tools) != 3 {
		t.Errorf("Se esperaban 3 herramientas expuestas, se obtuvieron: %d", len(listResult.Tools))
	}
...
func TestMCPServer_ToolExecution(t *testing.T) {
	...
	_, err = store.SaveObservation("Golang", "Go uses channels for goroutine communication.", "https://go.dev")
	if err != nil {
